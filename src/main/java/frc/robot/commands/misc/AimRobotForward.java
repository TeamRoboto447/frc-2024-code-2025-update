...
public class AimRobotForward extends Command {
    ...
    public AimRobotForward(SwerveSubsystem swerveSubsystem) {
    ...
    private Translation2d getForward(Alliance alliance) {
        double y = this.swerve.getPose().getTranslation().getY();
        if (alliance == Alliance.Blue) {
            return new Translation2d(0, y);
        } else {
            return new Translation2d(17, y);
        }
    }
    ...
    public void execute() {
        ...
        if (maybeAlliance.isPresent()) {
            target = getForward(maybeAlliance.get());
        }
